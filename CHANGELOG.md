...
### Added
- **Programs tab — install standard apps via Chocolatey.** A new **Programs** tab lets you push common Windows apps — Chrome, Firefox, Edge, Adobe/Foxit Reader, LibreOffice, 7-Zip, Notepad++, VLC, Zoom, Teams, TeamViewer, AnyDesk and more — to **one device or all your Windows machines** with a click. The agent bootstraps Chocolatey if it isn't present, then installs/upgrades the selected packages (idempotent). It also **shows what's already installed**, read from each device's reported software: *Installed / Not installed* for a single device, or *N/M installed* across a target. Installs run in the background and land in the device's run history. (Chocolatey is Windows-only; the target picker lists only online Windows devices.)
