...
- **Vendored agent synced to v2.2.14** — the agent bundled in the server image (served via `agent.zip`) now matches the canonical agent: cert pinning, per-device secret, login/lock-screen capture, the Ctrl+Alt+Del SAS fix, the winreg-based software scan, and silent one-shot screenshots.
