...
### Changed
- **Remote desktop now holds a steady 30 fps instead of a wandering number.** The frame rate used to jump around — 30 one moment, half that the next — and it turned out not to be the network. A screen grab waits for the display's next refresh, so grabbing a frame and *then* encoding it had to fit inside one refresh: the moment the pair took a fraction longer, the next grab had to wait for the refresh after that and the rate halved. Any small change in encoding cost flipped it back and forth between those two rates. Grabbing, encoding and sending now run at the same time as three stages, each frame is sent on a fixed clock rather than "whenever the last one finished", and a repeat of the last picture fills a tick where the display had nothing new (in H.264 that costs a few hundred bytes). Measured on a 2560×1440 screen: a flat **30.0 fps with no variation**, where the same machine previously bounced. The viewer asks for 30 now (all three quality presets do), and the agent holds it; where a device or a link genuinely can't, it settles on the highest rate it *can* hold — 24, 20, 15 — instead of oscillating, and climbs back on its own when there's room again. The frames/bandwidth readout in the viewer and the console is now averaged over three seconds, so it no longer reads 29/31/30 for a perfectly even stream. Requires agent **v2.2.45+** (and console **1.0.6+** for the readout).
- **Remote desktop is faster.** Sessions were running at 16–18 fps against a 20 fps target, and it wasn't the network — H.264 was using only 70–320 KB/s. It was the device doing needless work on every frame: copying the screen into an image, resizing it with a slow resampler, then copying it again before encoding. Frames now go straight from the screen grab into the video encoder, which converts and scales them in a single optimised pass — a third less work per frame (65.6 → 43.2 ms on a 2560×1440 screen), so sessions reach their 20 fps. On the JPEG fallback, a resize that would only shave a few percent off (a 2560 screen against a 2400 limit) is skipped, since resizing cost far more than it saved. Opening a session is also quicker: the server no longer starts a capture of its own right before the viewer starts the real one, which made the device launch and immediately discard a second capture process every time. Requires agent **v2.2.43+** for the frame changes.
- **The whole interface looks like one product.** The remote control page now wears the same header as Settings and Account (brand, breadcrumb, theme switch, alerts, your account) instead of its own bar with an underlined "Back" link. Its toolbar buttons each have their own icon — Copy and Paste used to share one, and Ctrl+Alt+Del had none. Every page now has the Leuffen shield as its browser-tab icon (the missing icon was also a 404 on every page load). The desktop console now uses the dashboard's own typefaces, Onest and JetBrains Mono, instead of falling back to Segoe UI (console **1.0.5+**). In **Settings → Authentication**, the explanation under the sign-in method no longer repeats the option's title and icon, so it no longer reads like a second option; and **Account** no longer shows your identity twice when your username is your email address.
- **Ctrl+C and Ctrl+V now work straight across a remote session.** They used to be passed through as plain hotkeys, so they acted only on the *remote* machine's own clipboard — text could only cross between the two computers with the **Copy**/**Paste** toolbar buttons, which is not what anyone's fingers expect. Now **Ctrl+V** pastes *your* clipboard onto the remote machine, and **Ctrl+C** (or Ctrl+X) copies on the remote and lands the result in *your* clipboard a moment later. It works the same in the browser viewer and in the desktop console, and the toolbar buttons stay for when you want to move the clipboard without pressing anything. In the browser this rides on the page's own paste event, so there's no clipboard permission prompt.
- **Downloads is split by who the download is for.** The panel used to be one long list in which the agent (which belongs on every customer device) and the new desktop console (which belongs only on a technician's own machine) sat side by side with nothing to tell them apart. It now has two labelled sections — **For your technicians** and **For the devices you manage** — each with a one-line explanation of where that software goes, and the console block is tinted so it can't be mistaken for another agent download. Within the agent section the customer-facing items are grouped together (enrolment key → MSI → shareable link → Synology → active keys) with the fleet-management controls (update agents, CPU temperature) after them.
...
### Fixed
- **Boxed notices on the dashboard have their frame back.** The style for these boxes was defined only for the Settings and Setup pages, so on the dashboard they rendered as bare, unframed text. That included the fresh enrolment key and the shareable install link. The style now lives in the shared stylesheet.
- **"Updates available" no longer reads as a percentage.** Its threshold is a number of updates, but the device panel and the incident history showed it as *"updates_available ≥ 1%"*. It now reads *"1 or more updates pending"*.
- **The device panel no longer repeats the OS version** when the operating system's name already includes it (*"Home Assistant OS 16.2 16.2"*).
- **The one-click installer link now configures the agent by itself.** Installing from a shared link still left people typing the server address and key into the tray by hand. The installer was passing both correctly — the agent simply couldn't see them: it was started before Windows had written them, and a scheduled task keeps the environment Windows had at boot, so newly written settings stay invisible to it until a reboot. It waited, re-reading a snapshot that could never change, then gave up and stayed offline. The agent now reads those settings straight from where Windows stores them, so it picks them up the moment they land, and the installer starts it only after writing them. Requires agent **v2.2.43+**; devices already stuck this way come up on their next reboot.
- **Settings no longer tells a global admin they aren't one.** The page loads three things at once, and if *any* of them failed — a server restart, a network blip, the database race fixed below — it said *"Admins only"*, which a refresh then appeared to fix. It now only says that on a real permission refusal; anything else is retried, and if it keeps failing the page says it couldn't load and offers **Try again**.
- **The agent download stopped working after a console release.** Servers that fetch the agent installer from GitHub's "latest release" were handed the desktop console's release, which has no agent installer — so every agent download and auto-update failed. Console releases no longer claim "latest".
- **The dashboard no longer throws random 500s when several requests arrive at once.** Every database read shared a single SQLite connection, and the web framework runs requests on a pool of threads — so two reads landing together could collide and fail with an internal error. In practice that meant a page (most often the fleet overview) intermittently failing to load with several browser tabs open, or with more than one person using the dashboard. Each thread now gets its own connection, with writes still serialised; a burst of 168 concurrent requests that previously failed around 5% of the time now fails none.
- **Creating an API key failed with an internal error.** The key hashing helper referenced `hashlib` without importing it, so every attempt to create a key crashed — the feature could never have worked.
- **The agent MSI download no longer breaks when a console build is published.** Both the agent and the desktop console ship from the same GitHub repo, so publishing the first console release made *"the latest release"* a console release — and the agent installer link, which pointed at `releases/latest/download/leuffen-rmm-agent.msi`, started returning **404** for every download *and* every in-place agent auto-update. Each download is now resolved by **asset name**: the server finds the newest release that actually carries that MSI, so the agent link follows `v*` releases and the console link follows `console-v*` releases regardless of which was published last. The Downloads panel also showed the console's version and size on the agent card for the same reason; both now show their own build.
- **Saving agent settings ("cannot reach server") works again.** The agent's Settings dialog verifies connectivity by probing `<server-url>/health` before it saves — but the server only served `/api/health`, so `/health` returned 404 and the dialog refused to save with a *"cannot reach server"* error (and, while the server was busy proxying the big MSI download, that probe got *"remote end closed connection without response"*). The server now also serves **`/health`** (200), so the connection check passes and settings save. No new agent needed — server update only.
- **Agent (MSI) download link works again.** The installer download proxied the whole ~60 MB MSI **through the server into memory** before sending it, which was heavy enough that the connection dropped before the client got a response — *"cannot reach server / remote end closed connection without response"* on every machine, even though the server was up. The download now **redirects straight to the (public) GitHub release**, so the client fetches the MSI from GitHub's CDN and the server does no proxying. Both the one-click installer and the direct MSI link work.
- **The agent now recovers on its own when it stops.** The agent's Windows scheduled task only restarted it **at boot**, and ran under Task Scheduler's default **72-hour execution limit** — so a crashed or stopped agent stayed offline **until the next reboot** (a real case: an always-on server's agent exited after an update and was offline for days), and a long-running one could even be killed after 72h. On every startup the agent now re-asserts its task to **restart on failure (every 5 min) with no execution-time limit**, so a dead agent comes back within minutes without a reboot. Requires agent **v2.2.42+** (rolls out on the next auto-update; you can also apply it immediately to an existing machine with a one-off `Set-ScheduledTask`).
...
### Added
- **Home Assistant OS servers can be monitored.** The agent now also runs as a Home Assistant **add-on**. **Downloads → Home Assistant OS** has a one-click link that adds the add-on repository to your Home Assistant, and a button that lays out the server address and a fresh enrolment key exactly as the add-on's Configuration tab asks for them, each with its own copy button. Once approved, the server shows up like any other device, with its own house icon. It reports CPU, memory, the data disk, temperature, uptime and network traffic, plus:
  - pending **updates** for Home Assistant OS, Core, the Supervisor and each add-on — the existing **Updates available** monitor alerts on them;
  - whether **Home Assistant itself and each add-on** are running — they appear under **Services**, so the existing **Service not running** monitor can watch, say, Mosquitto or Zigbee2MQTT;
  - installed add-ons under **Software**.

  **Reboot** and **shut down** work remotely. **Files** opens the Home Assistant configuration folder, and **Terminal** runs inside the add-on. Remote desktop isn't available, because Home Assistant OS has no desktop. Add-on updates arrive through Home Assistant's own add-on store, whenever a new agent is released.
- **The desktop console keeps itself up to date.** It asks the server for the newest published build at start-up and every six hours, and shows a banner when one is available. One click downloads it and starts the installer; the console closes so the new version can replace it. It offers rather than installing silently on purpose — the console installs per machine, so Windows asks for permission once, on the click.
- **Sign-in can be restricted to the networks you name.** **Settings → Security** gained an **allow list** and a **block list** (single addresses or CIDR ranges, one per line, `#` comments allowed). A block rule always wins; the moment the allow list has one entry, only those addresses can sign in. It covers the sign-in page, the desktop console and the `/api/v1` integration API — **agents are never filtered**, so a mistyped rule can't take your fleet offline. Rules that would shut *you* out are refused when you save, and the page shows the address the server actually sees you arrive from, which is also how you tell whether **behind a reverse proxy** needs turning on (without it every caller looks like the proxy).
- **Admins can be emailed about repeated failed sign-ins.** Switch it on in **Settings → Security** and pick the threshold and a quiet period — after an alert, that same address-and-account stays silent for the configured minutes, so a sustained attack can't fill your inbox. Mail goes to your alert recipients, or to every global admin if none are configured.
- **Two-factor can be required of everyone, Microsoft 365 included.** The **Require two-factor** switch (Settings → Authentication) previously only reached password accounts. It now also covers people who sign in through Microsoft 365: they are sent to set up an authenticator before they can use the dashboard, on top of whatever your tenant already asks for. An SSO identity that never had a local account gets one created to hold its secret and recovery codes — sign-in still goes through Microsoft.
- **Paste as keystrokes in remote control.** Some places simply refuse a paste — UAC prompts, the Windows sign-in screen, a remote session running inside the remote session, password boxes that block pasting. The viewer can now **type** your clipboard out instead, character by character, which is indistinguishable from someone at the keyboard and therefore always lands. Use the **Paste as keystrokes** button or **Ctrl+Shift+V**; plain Ctrl+V still does a normal clipboard paste. Works in the browser viewer and the desktop console, capped at 10,000 characters because typing is much slower than pasting.
- **"Keep me signed in for 30 days" on the sign-in page.** Signing in used to last only until you closed the browser, so every morning started with the password. Tick the box and the session is kept for 30 days instead — on the local sign-in form and on the Microsoft 365 button alike. Leave it unticked and nothing changes: the session still ends when the browser closes. Session cookies now also carry the moment they were issued and are refused by the server after 30 days, so a cookie that leaks somewhere is no longer valid forever (it previously had no expiry at all). Existing sessions keep working; the lifetime is configurable with `RMM_SESSION_DAYS`.
- **Settings → API & webhooks: give another system a way in.** A global-admin section for the integration surface that had a backend but no way to reach it. Issue an **API key** and choose its **scope**: leave it on *All organisations* and one key reads your whole estate — what a shared ticket system wants — or pin it to one organisation and it sees only that customer's devices and alerts. Keys are shown **once** on creation (only a hash is stored), can be **suspended** without being destroyed, and show when they were last used. Alongside it, **webhooks**: Leuffen RMM posts signed JSON the moment an alert is raised or clears, which is enough for a ticket system to open a ticket and close it again by itself — with a **Test** button that reports what your endpoint actually answered. Every delivery carries `X-RMM-Signature: sha256=…`, an HMAC of the exact body. The API lives at `/api/v1` (`devices`, `devices/{id}`, `alerts`, `run-script`, `reboot`) and takes the key as `X-API-Key` or `Authorization: Bearer`.
- **Desktop console — remote control as a real Windows program (the browser viewer stays).** Remote support no longer has to live in a browser tab. A new **Leuffen RMM Console** installs on a technician's own machine and gives you the same sessions natively: **remote control**, **terminal** and **file transfer**, plus a searchable device list per organisation with power actions. Both front-ends drive the **same** `/api/devices/{id}/screen` bridge on the same server, so they are two ways into one session — nothing changes on the managed device, and the web viewer keeps working exactly as before.
  - **Choose per session.** Each device's **Remote control** button gained a chevron: *In this browser* or *In the desktop app*. Your choice becomes the default for next time.
  - **Sign-in without retyping anything.** "Open in desktop app" mints a **single-use, two-minute ticket** and hands it to the app through a `leuffenrmm://` link, so the app is signed in by the browser session you already had — which is also how **Microsoft 365 SSO** users get in without a password ever being typed into the app. Local accounts can sign in directly in the app instead (including the authenticator code), and the token is stored **encrypted with Windows DPAPI**, not in a plain file.
  - **Self-signed servers in one tick.** Trusting the server's certificate also **pins** it (SHA-256, shown for verification), so a self-signed deployment stays MITM-proof — the same trade-off the agent makes with `RMM_SERVER_FINGERPRINT`.
  - **Better than the tab where it counts.** Real keyboard capture (Alt+Tab, Win, Ctrl+W reach the *remote* machine instead of the browser), no clipboard permission prompt, and frame pacing that paints only the newest frame so a slow link can't build a backlog of stale screens. H.264 is used when available, with the same JPEG fallback as the web viewer.
  - Download it from **Downloads → Desktop console** (install it on **your own** machine, not on managed devices). It ships from the agent repo as its own MSI and updates independently of the agent.
- **Programs tab — install & keep standard apps updated via Chocolatey.** A new **Programs** tab lets you push and maintain common Windows apps across your fleet:
  - **Curated catalog** — Chrome, Firefox, Edge, Adobe/Foxit Reader, LibreOffice, Teams, Zoom, Slack, Discord, VS Code, Git, 7-Zip, Notepad++, Greenshot, PowerToys, VLC, TeamViewer, AnyDesk, Google Drive. The agent bootstraps Chocolatey if needed, then installs/upgrades the selected packages (idempotent).
  - **Flexible targets** — a single device, a **group**, **all Windows desktops**, **all Windows servers**, or **all Windows devices**.
  - **Shows what's already installed** (from each device's reported software): *Installed / Not installed* for one device, or *N/M installed* across a target.
  - **Update all** — run `choco upgrade all` on the target on demand, and an **Auto-update** toggle (per organisation) that keeps Chocolatey apps up to date **nightly**, only upgrading what actually has an update.
  - Installs/updates run in the background and land in the device's run history. (Chocolatey is Windows-only; targets list only online Windows devices.)
